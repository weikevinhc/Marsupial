...
plot(example$pos, example$d/100, type = "l", col = "red",
		 ylab = "Genetic distnace (M)/Recombinant fraction", xlab = "positions (Mb)")
...
So, now we have a r2d object (example, example2, and example3), let's try to simulate crossovers with them. We will first generate a pool of 100 individuals and observe the allele frequnecy in them.
```{r eval=T,fig.height = 4, fig.width = 4}
...
Now let's try simulating what the AF from 1000 individuals will look like if we sequneced it. Let's assume males are pooled and are sequenced to a coverage of 30x.
```{r eval=T,fig.height = 4, fig.width = 4}
